    ...
    <section class="space-y-6">
        ...
        <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            <article class="panel-card p-5">
                ...
                <p class="mt-2 text-4xl font-black tracking-tight text-slate-900">PHP {{ number_format($stats['total_billed'], 2) }}</p>
            </article>
            <article class="panel-card p-5">
                <p class="muted-label">Paid Amount</p>
                <p class="mt-2 text-4xl font-black tracking-tight text-emerald-600">PHP {{ number_format($stats['paid_amount'], 2) }}</p>
            </article>
            <article class="panel-card p-5">
                <p class="muted-label">Pending Amount</p>
                <p class="mt-2 text-4xl font-black tracking-tight text-amber-600">PHP {{ number_format($stats['pending_amount'], 2) }}</p>
            </article>
            <article class="panel-card p-5">
                <p class="muted-label">Total Invoices</p>
                <p class="mt-2 text-4xl font-black tracking-tight text-slate-900">{{ number_format($stats['total_invoices']) }}</p>
            </article>
        </div>
